...
def make_plot_summary(
    normalized: pd.DataFrame, baseline_hour: float
) -> pd.DataFrame:
    """Calculate the exact normalized mean and SEM sent to Matplotlib."""
    summary = (
        normalized.groupby(["sample", "elapsed_hours"], dropna=False)
        .agg(
            n=("series_id", "nunique"),
    ...
    if baseline.empty or not np.allclose(baseline[PLOT_MEAN_COLUMN], 1.0, atol=1e-12, rtol=0.0):
        raise RuntimeError(
            f"Refusing to plot: sample means are not all 1.0 at baseline hour {baseline_hour:g}."
        )
...
def add_log2_fold_change(
    ...
    if not np.allclose(baseline_values, 0.0, atol=1e-12, rtol=0.0):
        raise RuntimeError(
            f"Internal check failed: every per-well log2 value must be 0 at baseline hour {baseline_hour:g}."
        )
    return out
...
def make_log2_plot_summary(
    ...
    if baseline.empty or not np.allclose(
        baseline[LOG2_PLOT_MEAN_COLUMN], 0.0, atol=1e-12, rtol=0.0
    ):
        raise RuntimeError(
            f"Refusing to plot: log2 sample means are not all 0 at baseline hour {baseline_hour:g}."
        )
    return summary
...
def plot_sample(
    ...
    *,
    ...
    x = sub["elapsed_hours"].to_numpy(dtype=float)
    y = sub[mean_column].to_numpy(dtype=float)  # The only y array sent to ax.plot.
    sem = sub[sem_column].to_numpy(dtype=float)
...
def main(argv: list[str] | None = None, *, prog: str | None = None) -> int:
    ...
    print(
        f"Created {2 * len(plot_definitions)} PNG-only plot(s): "
        f"linear and log2 versions of {len(plot_definitions)} layout plot(s) "
